...
test("Calculates the yearly and monthly premium based on the car value and risk rating", () => {
  ...
  expect(qoute.yearlyQuote).toBe(720);
  expect(qoute.yearlyQuote).toBe(60);
});

test.todo("Test with various car values and risk ratings within valid ranges", () => {
...
test.todo("Test the lowest and highest risk rating values", () => {
...
test.todo("Returns an error if the risk rating is missing, out of range, or not a valid number", () => {
  const outOfRange = 7;
...
test.todo("Returns an error if the car value is missing or not a valid number", () => {
...
console.log(
    `${CAR_YEAR} ${CAR_MAKE} ${CAR_MODEL} is in ${riskRatingResult} vehicle. Yearly premium: $${yearlyQuote} and Monthly premium: $${monthlyQuote}`
  );